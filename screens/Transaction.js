...
export default class TransactionScreen extends Component {
  ...
  checkStudentEligibilityForBookIssue = async () => {
    ...
    var stRef = await db.collection("students").where("student_id", "==", this.state.studentId).get()
    if(stRef.docs.length === 0){
     isStudentEligible = false;
     Alert.alert("Oh no... this student does not exist!")
     this.setState({
       bookId: "",
       studentId : ""
     })
      ...
      stRef.docs.map(doc =>{
        isStudentEligible = doc.data().number_of_books_issued < 2? true:(false,   this.setState({
          bookId: "",
          studentId : ""
        }), Alert.alert("Oh no... looks like you have too many books.. :("))
      })
  ...
  checkStudentEligibilityForBookReturn = async () => {
    ...
    const studentRef = await db
      ...
      .where("book_id", "==", this.state.bookId).limit(1)
      .get();
